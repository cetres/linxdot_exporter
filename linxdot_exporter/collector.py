...
from .exceptions import (LoginExpiredException, LoginFailedException,
                         RequestException)
...
class LinxdotCollector(object):
    ...
    def __init__(self, host: str, username: str, password: str,
                 refresh_interval: Optional[int] = None, secure=False) -> None:
    ...
    def login(self) -> None:
        ...
        logging.info(f'Login at url {url} with username '
                     '{self._login_params["username"]}')
        r = self.sess.post(url, data=self._login_params)
        if r.status_code != 200:
            logging.warn(f'Login failed at {url} - HTTP error code: '
                         '{r.status_code}')
